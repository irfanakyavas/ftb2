...
class MatchDataScraper:
    ...
    def __init__(self, driver_type: DriverType):
        ...
        if driver_type == MatchDataScraper.DriverType.FIREFOX:
            Logger.ScrapingLogger.log(level=logging.INFO,
                                      msg="Firefox Web Driver was chosen for Selenium, trying to initialize.")
            try:
                self.driver = selenium.webdriver.Firefox()
            except selenium.common.exceptions.WebDriverException:
                Logger.ScrapingLogger.log(level=logging.CRITICAL,
                                          msg="COULD NOT initialize Firefox Web Driver, is Firefox Web Driver present in path variable?.")
            Logger.ScrapingLogger.log(level=logging.INFO, msg="Firefox Web Driver was successfully initialized.")

        # Logging & initialization for Chrome webdriver
        if driver_type == MatchDataScraper.DriverType.CHROME:
            Logger.ScrapingLogger.log(level=logging.INFO,
                                      msg="Chrome Web Driver was chosen for Selenium, trying to initialize.")
            try:
                self.driver = selenium.webdriver.Chrome()
            except selenium.common.exceptions.WebDriverException:
                Logger.ScrapingLogger.log(level=logging.CRITICAL,
                                          msg="COULD NOT initialize Chrome Web Driver, is Chrome Web Driver present in path variable?.")
            Logger.ScrapingLogger.log(level=logging.INFO, msg="Chrome Web Driver was successfully initialized.")

        # Logging & initialization for Opera webdriver
        if driver_type == MatchDataScraper.DriverType.OPERA:
            Logger.ScrapingLogger.log(level=logging.INFO,
                                      msg="Opera Web Driver was chosen for Selenium, trying to initialize.")
            try:
                self.driver = selenium.webdriver.Opera()
            except selenium.common.exceptions.WebDriverException:
                Logger.ScrapingLogger.log(level=logging.CRITICAL,
                                          msg="COULD NOT initialize Opera Web Driver, is Opera Web Driver present in path variable?.")
            Logger.ScrapingLogger.log(level=logging.INFO, msg="Opera Web Driver was successfully initialized.")
    ...
    def scrape_league(self, league: League, scrape_range: slice = slice(0, None),
