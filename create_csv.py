...
def get_lineups(id):
    cursor.execute("SELECT * FROM matches WHERE match_id="+str(id))
    result = cursor.fetchall()
    for i in result:
        home_name, away_name = i[1], i[2]
        home_goals, away_goals = i[3], i[4]
        if home_goals > away_goals:
            ...
            match_result = -1
        str_lineup = i[37], i[38]
        home_lineup, away_lineup = json.loads(str_lineup[0]), json.loads(str_lineup[1])
        return home_name, away_name, home_lineup, away_lineup, match_result
